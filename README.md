...
6. **Container Build**
   ...
   - The reusable workflow is separate and only publishes to GitHub Container Registry when a trusted caller sets `with.push_image: true` from a `push` event on `main`
...
3. **Container Build**
   ...
   - The reusable self-hosted workflow is the opt-in path that uses `with.push_image: true` from a `push` event on `main` to publish to GitHub Container Registry
